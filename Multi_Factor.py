...
def get_candidate_list(context):
    ...
    # 因子包括：market_cap，pe_ratio，pb_ratio，return_on_invested_capital，inc_revenue，inc_profit_before_tax
    #取沪深300作为股票池
    feasible_stocks = set_feasible_stocks(context)
    ...
    # for i in range(0,1):
        ...
        if factor_i == factor_refer_name:
            # 获取因子值
            factor = get_factor(context,factor_i,context.current_dt - dt.timedelta(days=1),backward_i)
            #去极值
            factor = mad(factor)
        ...
        # 如果不是市值因子，则进行中性化，中性化后进行标准化
            ...
            # 提取参考的市值因子
            factor_refer = get_factor(context,factor_refer_name,context.current_dt - dt.timedelta(days=1),1)
...
def get_factor(context,factor_name,date,if_backward):
    #获取五张财务基础所有指标名称
    ...
    stock = set_feasible_stocks(context)
